...
- ログは `~/Library/Logs/Sidekick/sidekick.log` のみに、ユーザーだけが読める権限と容量ベースのローテーション付きで保存されます。
- 通知プレビューにはフィードバックが届いたことだけを表示し、画面内容をもとにモデルが生成した本文は表示しません。
...
ビルドスクリプトはリポジトリ内に `dist/Sidekick.app` を作り、ローカルテスト用に ad-hoc signing します。ad-hoc 版は公証されていないため、Gatekeeper の警告が表示される場合があります。`/Applications` には自動インストールしません。必要なら手動で移動してください。
...
テストは `swift test` で実行できます。Sidekick が平文 HTTP で接続できるのは `localhost` や `127.0.0.1` などのループバックアドレスだけで、外部の接続先には HTTPS が必要です。通知のプレビューにはモデルの応答本文を表示しません。ログはユーザーだけが読める権限で `~/Library/Logs/Sidekick/sidekick.log` に保存し、容量に応じてローテーションします。

公開版は Developer ID Application 証明書で署名し、Apple の公証を受ける必要があります。`xcrun notarytool store-credentials` で App Store Connect の認証情報をキーチェーンへ登録してから、次を実行します。
...
署名・公証の認証情報は開発者のキーチェーンまたは CI のシークレットに保存し、リポジトリへコミットしないでください。
